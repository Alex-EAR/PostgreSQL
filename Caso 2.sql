...
from (select * 
	  from employees join (select *
						  from departments
						  where departments.department_id = 100) as dpt
	  on employees.department_id = dpt.department_id) as empl;
--5
select *
from departments
where departments.department_id in (10,70);
--6
...
from employees join (select *
					from departments
					where departments.department_name in ('Finance','Shipping')) as dpt
	on employees.department_id = dpt.department_id;